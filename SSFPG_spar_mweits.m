function [X, misfit, t0, t_isrevise, Xall] = SSFPG_spar_mweits(G, D, weits, ob, iter, evalchar, isscaling)  % 多重权重一次求解版
%==========================================================================
...
% 求 G'*G + D'*D*weits² 的最大特征值（原用幂迭代法，收敛慢）
...
% 下面预分配记录数组
%Gob = G' * ob(1:sg(1)) + D' * ob(1 + sg(1):end);  % 【原注】投影梯度相关量，未使用
%GobE = Gob' * Gob;                            % 【原注】G'*ob 的能量，未使用
misfit = zeros(iter, numel(weits));            % 归一化残差记录：行=迭代次数，列=权重
obE = ob' * ob;                                % 观测数据能量，用于归一化残差
%numstop = 0;                                  % 【原注】用于提前停止的计数器，未启用
% 初始化解存储与尺寸信息
sg = size(G);                                  % 取 G 的尺寸 [数据行数 模型参数个数]
Xall = zeros(sg(2), numel(weits), iter);       % 三维数组存放所有解：解维度 × 权重个数 × 迭代次数
% 下面计算初始残差与初始梯度
X = zeros(sg(2), numel(weits));                % 初始解取全 0 矩阵（列数=权重个数）
%ex = ob - [G * X; D * X];% sg(1)*sg(2)        % 【原注】未加权时的残差写法
ex = ob - [G * X; D * (X .* weits(:)')];       % 初始残差：约束块按列乘上各自的权重【原文：sg(1)*sg(2)】
% 初始梯度：D' 作用后再按列乘权重（与加权残差配套）【原文：sg(1)*sg(2)】
Xe = G' * ex(1:sg(1), :) + (D' * ex(sg(1) + 1:end, :)) .* weits(:)';
...
for i = 1:iter                                 % 逐次迭代，共 iter 次
    ...
    eval(evalchar);                            % 对全体权重的解执行投影
    % 计算合成数据
    %syn = [G * X; D * X]; fac = bestfac(ob, syn); syn = syn * fac; X = X * fac; ex = ob - syn;  % 【原注】未加权写法
    ...
    % 用新残差更新下降方向（约束块按权重加权）【原文：sg(1)*sg(2)】
    Xe = G' * ex(1:sg(1), :) + (D' * ex(sg(1) + 1:end, :)) .* weits(:)';
...
% function egmax = egmax_esti_mweits(G, D, weits, kk)  % 【原注】幂迭代估计多权重最大特征值
%
% m = size(G, 2);                              % 【原注】模型参数个数
%
% numweits = numel(weits);                     % 【原注】权重个数
% x = randn(m, numweits);                      % 【原注】随机初始矩阵
% x = x ./ sqrt(sum(x .^ 2));                  % 【原注】逐列归一化
%
% for i = 1:kk                                 % 【原注】幂迭代循环
%     x = G' * (G * x) + D' * (D * x) .* weits(:)' .^ 2;  % 【原注】作用一次加权算子
%     x = x ./ sqrt(sum(x .^ 2));              % 【原注】逐列归一化
% end                                          % 幂迭代结束
% egmax = sum([G * x; (D * x) .* weits(:)'] .^ 2);  % 【原注】最大特征值估计
% 以下为本文件内的局部子函数：批量 Lanczos 法估计多权重最大特征值
function [L, info] = GD_mweits(G, D, weights, maxit, tol)  % 子函数声明：同时估计多个权重下 [G; w*D]'*[G; w*D] 的最大特征值
% Estimate lambda_max of [G; w*D]'*[G; w*D]    % 估计 [G; w*D]'*[G; w*D] 的最大特征值
% simultaneously for multiple weights using batched Lanczos.  % 用批量 Lanczos 法对多个权重同时求解
%
% INPUT                                        % 输入
%   G       : dense N-by-M matrix              % 稠密矩阵 G，N×M
%   D       : sparse P-by-M matrix             % 稀疏矩阵 D，P×M
%   weights : vector [w1,w2,...,wK]            % 权重向量
%   maxit   : maximum Lanczos iterations, default 12  % 最大 Lanczos 迭代次数，默认 12
%   tol     : relative Ritz residual tolerance, default 1e-3  % 相对 Ritz 残差容差，默认 1e-3
%
% OUTPUT                                       % 输出
%   L       : lambda_max(G'*G + w^2*D'*D) for all weights  % 各权重的最大特征值
%             = sigma_max([G;w*D])^2           % 亦等于 [G;w*D] 最大奇异值的平方
%   info.iter                                  % 实际迭代次数
%   info.relres                                % 相对残差
%
% The expensive G'*G and D'*D matrices are never formed.  % 全程不显式构造昂贵的 G'*G 与 D'*D 矩阵
% 参数默认值处理
...
% 权重整理
w = weights(:).';                              % 权重整理为行向量 1×K
K = numel(w);                                  % 权重个数 K
w2 = w .^ 2;                                   % 预先算好权重的平方，供算子中使用
%% Initial vectors                             % 初始化向量
...
for j = 1:maxit                                % 逐个 Lanczos 步迭代
    %% ---------------------------------------------------------  % 批量算子作用说明
    % Batched application of:                  % 批量作用下述算子
    %
    % B_k q_k = G'G q_k + w_k^2 D'D q_k        % 算子定义（每个权重一个）
    %
    % No loop over weights in the expensive operations.  % 昂贵的矩阵运算中不做权重循环（批量化）
    % ----------------------------------------------------------  % 说明结束
    GQ = G * Q;                                % 预先计算 G*Q，供 G'*GQ 复用
    DQ = D * Q;                                % 预先计算 D*Q，供 D'*(DQ.*w2) 复用
    Z = G' * GQ + D' * (DQ .* w2);             % 批量作用算子 B：G'GQ + w²(D'DQ)，一次算完所有权重
    %% Lanczos three-term recurrence           % Lanczos 三项递推
    if j > 1                                   % 首轮没有上一轮分量，跳过
        Z = Z - Qold .* beta_old;              % 减去上一轮方向的分量（三项递推的第三步）
    end                                        % 递推分支结束
    aj = sum(Q .* Z, 1);                       % 计算当前方向上的投影（三对角对角元 alpha_j）
    Z = Z - Q .* aj;                           % 减去当前方向的分量，得到新正交方向
    % Small local reorthogonalization          % 小的局部再正交化，抑制数值正交性丢失
    if j > 1                                   % 首轮无需再正交化
    ...
    alpha(j, :) = aj;                          % 记录对角元
    beta(j, :) = bj;                           % 记录次对角元
    %% Ritz values                             % 求 Ritz 值
    % Only tiny j-by-j matrices are handled in this loop.  % 此循环只处理极小的 j×j 矩阵
    % Its cost is negligible compared with G*Q and G'*GQ.  % 其开销相比 G*Q、G'*GQ 可忽略
    ...
    %% Stop when all weights have converged    % 所有权重都收敛后停止
...
% Preserve input shape                         % 保持输出形状与输入权重形状一致
