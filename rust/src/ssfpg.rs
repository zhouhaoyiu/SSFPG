//! SSFPG：基础版（对应 MATLAB 的 `SSFPG.m`）——系数矩阵只有稠密块 `G`，单组观测。
//!
//! 导出 C 符号 `ssfpg_run_dense`。与 `ssfpg_spar.rs` 的唯一区别是初始解取全 1
//! （而不是全 0），其余迭代逻辑相同；稀疏块在本文件里退化为空（`p = 0`）。
...
fn run(
    ...
    let mut x = vec![1.; m * k]; // 基础版初始解取全 1
...
#[no_mangle]
pub unsafe extern "C" fn ssfpg_run_dense(
    n: usize,
    m: usize,
    p: usize,
    k: usize,
