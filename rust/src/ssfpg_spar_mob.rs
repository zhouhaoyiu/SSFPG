//! SSFPG_spar_mob：多组观测数据同时反演（对应 MATLAB 的 `SSFPG_spar_mob.m`）。
//!
//! `ob` 可以是多列矩阵（每列一组独立观测），整批数据共用同一条步长序列。
//! 与 `ssfpg_spar.rs` 的区别在于回退方式：残差上升的列不再做 40 点对数搜索，
//! 而是直接改用固定的 `1.5/L` 收缩步长。
//! 导出 C 符号 `ssfpg_run_mob`。
...
#[no_mangle]
pub unsafe extern "C" fn ssfpg_run_mob(
    n: usize,
    m: usize,
    p: usize,
    k: usize,
