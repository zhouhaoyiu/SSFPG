function [X_out, misfit, t0, t_isrevise, Xall] = SSFPG_spar_mob(G, D, ob, iter, evalchar, isscaling)  % 多组观测同时反演版
%==========================================================================
...
%% get the maximum eigenvalue of G'*G+D'*D     % 求 G'*G+D'*D 的最大特征值
...
%GobE = sum(Gob .^ 2);                         % 【原注】G'*ob 的能量，此处未使用
misfit = zeros(iter, size(ob, 2));             % 归一化残差记录：行=迭代次数，列=数据组
obE = sum(ob .^ 2);                            % 各组观测数据的能量（按列求和），用于归一化残差
% 初始化解存储与尺寸信息
sg = size(G);                                  % 取 G 的尺寸 [数据行数 模型参数个数]
Xall = zeros(sg(2), size(ob, 2), iter);        % 三维数组存放所有解：解维度 × 数据组数 × 迭代次数
% 下面计算初始残差与初始梯度
X = zeros(sg(2), size(ob, 2));                 % 初始解取全 0 矩阵（列数=数据组数）
ex = ob - [G * X; D * X];                      % 初始残差：ob 与堆叠正演结果 [G*X;D*X] 之差【原文：sg(1)*sg(2)】
Xe = G' * ex(1:sg(1), :) + D' * ex(sg(1) + 1:end, :);  % 初始梯度：两块残差分别经 G'、D' 作用后按列相加【原文：sg(1)*sg(2)】
...
for i = 1:iter                                 % 逐次迭代，共 iter 次
    ...
    eval(evalchar);                            % 对整批解执行可行域投影（如非负投影）
    % 计算合成数据
    ...
    % if t0(i) exceeds tmax, misfit(i) needs t0 be smaller than misfit(i - 1)  % 【原注】步长过大时残差必须下降
    if t0(i) > tmax                            % 若当前步长超过保证收敛的上界
        ...
        if numel(index) > 0                    % 若存在需要重搜的数据组
            ...
            %X_rea = zeros([size(X, 1), 20, size(X_re, 2)]);  % 【原注】批量试探解缓存，未使用
            % 用一个缩小后的固定步长重新试算
...
%                 X_rea0(X_rea0 < 0) = 0;      % 【原注】非负投影
%                 syn = [G * X_rea0; D * X_rea0];  % 【原注】正演
%                 if isscaling == 1            % 【原注】若开启缩放
%                     fac = sum(ob(:, index(kk)) .* syn) ./ sum(syn .* syn);  % 【原注】计算缩放因子
%                     fac = fac(:)'; syn = syn .* fac; X_rea0 = X_rea0 .* fac;  % 【原注】缩放合成数据与解
%                 end                          % 缩放分支结束
%                 ex0 = ob(:, index(kk)) - syn;  % 【原注】计算残差
%                 misfitj = sum(ex0 .^ 2) ./ obE(index(kk));  % 【原注】计算归一化残差
%                 [~,nj]=min(misfitj);         % 取残差最小的候选
...
%             end                              % 循环结束
    ...
    % 更新梯度并保存本轮解
    Xe = G' * ex(1:sg(1), :) + D' * ex(sg(1) + 1:end, :);  % 用新残差更新整批数据的下降方向【原文：sg(1)*sg(2)】
    Xall(:, :, i) = X;                         % 把本轮解存入 Xall 的第 i 层（同一批全部数据组）
end                                            % 主迭代循环结束
% get the solution with the minimum misfit     % 取残差最小的解作为结果
