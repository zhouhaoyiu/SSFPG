function [X_out,misfit,t0,t_isrevise,Xall,Xdif]=SSFPG(G,ob,Xtol0,iter,evalchar,isscaling)  % 函数声明：SSFPG 主求解器，解非负（或带界）约束线性最小二乘
%==========================================================================
...
egmax=egmax_esti(G);% more efficient                        % 高效估计 G'*G 的最大特征值（调用本文件末尾的子函数）
% 下面准备迭代所用的松弛（步长）序列
t0=load('t0_1e6_30.mat');% pre-calculated step sizes        % 载入预先计算好的步长序列（.mat 中的变量 t0）
t0=t0.t0/egmax; % scaled by the Lipschitz constant           % 用 Lipschitz 常数对步长做归一化缩放
tmax=2/egmax;                                                % 保证收敛的步长上界 2/egmax（投影梯度法的经典上界）
repnum=ceil(iter/numel(t0));                                 % 计算步长序列需要重复多少遍才能覆盖全部迭代次数
t0=repmat(t0,[repnum,1]); % repeated use                      % 若迭代次数多于预设步长个数，则把序列重复拼接后循环使用
% 下面预分配迭代过程中需要记录的各数组
%Gob=G'*ob;                                                   % （保留原注释）投影梯度相关量 G'*ob，此处未使用
%GobE=Gob'*Gob;                                               % （保留原注释）G'*ob 的能量，此处未使用
Xall=zeros(sg(2),iter);% solutions in all iterations          % 存放所有迭代的解：每列对应一次迭代的解向量
% 初始化解向量
X=ones(sg(2),1); % initial solution                          % 初始解取全 1 向量（长度为模型参数个数）
% 初始化残差与变化量记录数组
misfit=zeros(iter,1); % relative misfit                      % 归一化残差记录数组
%misfite=zeros(iter,1); % relative misfit                    % （保留原注释）基于梯度的另一种残差指标，未启用
Xdif=zeros(iter,1);                                          % 相邻两次迭代解相对变化量记录数组
obE=ob'*ob; % energy of data                                 % 观测数据的能量（二范数平方），用于把残差归一化
% 下面计算初始残差与初始梯度
ex=ob-G*X;% sg(1)*sg(2)                                      % 初始残差 ex = ob - G*X
Xe=G'*ex;% sg(1)*sg(2)                                       % 初始梯度（最速下降方向）Xe = G'*ex
...
for i=1:iter                                                 % 逐次迭代，共 iter 次
    ...
    % 执行可行域投影
    eval(evalchar);% projection, e.g., non-negativity, upper bound limitation, smoothing  % 按用户指令投影（非负、上界、平滑等）
    % 计算合成数据与新残差
    syn=G*X;                                                 % 由当前解正演合成数据 syn = G*X
    if isscaling==1                                          % 若开启了“解缩放”选项
        fac=ob'*syn./(syn'*syn);syn=syn*fac;X=X*fac; % scaled for rapid converging        % 缩放因子 fac=(ob'syn)/(syn'syn)，同步缩放合成数据与解以加速收敛
    end                                                      % 解缩放分支结束
    ...
    if t0(i)>tmax                                            % 若当前步长超过了保证收敛的上界 2/egmax
        if i>1&&misfit(i)-misfit(i-1)>eps                    % 且非首次迭代、且残差反而变大（说明步长过大）
            ...
            % 对批量试探解做同样的投影
            eval(evalchar);% projection                      % 对全部试探解执行相同的可行域投影
            % 对批量试探解做正演
            syn=G*X;                                         % 批量正演合成数据
            if isscaling==1                                  % 若开启了“解缩放”选项
                fac=sum(ob.*syn)./sum(syn.*syn);             % 逐列计算缩放因子（对批量解按列求和）
                syn=syn.*fac;X=X.*fac;% scaled for rapid converging   % 对批量合成数据与批量解同步缩放
            end                                              % 批量缩放分支结束
            ex=ob-syn;                                       % 批量更新残差
            % 计算每个候选步长对应的残差
    ...
    Xe=G'*ex;% sg(1)*sg(2)                                   % 用新残差更新梯度，作为下一轮的下降方向
    % 下面一行是另一种停止判据，未启用
    %misfite(i)=Xe'*Xe/GobE;%(X-X0)'*(X-X0);                 % （保留原注释）基于梯度的残差指标
    Xall(:,i)=X;                                             % 把本轮解存入 Xall 的第 i 列
    % 收敛判据：当解基本不再变化时停止迭代
    % stop the iteration if the deepest misfit reduction (t0>tmax) has been stably small  % 原文注释
    ...
    if Xdif(i)<Xtol0                                         % 若小于给定容差，认为已收敛
        ...
        %misfite(i+1:end)=[];                                % （保留原注释）对应裁剪，未启用
