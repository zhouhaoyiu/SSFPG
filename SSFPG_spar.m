function [X_out,misfit,t0,t_isrevise,Xall,Xdif]=SSFPG_spar(G,D,ob,Xtol0,iter,evalchar,isscaling)  % 函数声明：系数矩阵由“稠密块 G + 稀疏块 D”拼接时的 SSFPG 求解器
%==========================================================================
...
egmax=egmax_esti(G,D);% more efficient estimation           % 高效估计 G'*G+D'*D 的最大特征值（调用本文件末尾子函数）
% 下面准备迭代所用的松弛（步长）序列
t0=load('t0_1e6_30.mat');% pre-calculated step sizes        % 载入预先计算好的步长序列
t0=t0.t0/egmax; % scaled by the Lipschitz constant           % 用 Lipschitz 常数对步长归一化缩放
tmax=2/egmax;                                                % 保证收敛的步长上界 2/egmax
repnum=ceil(iter/numel(t0));                                 % 计算步长序列需要重复多少遍
t0=repmat(t0,[repnum,1]); % repeated use                     % 迭代超长时把步长序列循环拼接使用
% 下面预分配迭代过程中需要记录的各数组
%Gob=G'*ob(1:sg(1))+D'*ob(1+sg(1):end);                       % （保留原注释）投影梯度相关量，此处未使用
%GobE=Gob'*Gob;                                               % （保留原注释）G'*ob 的能量，此处未使用
Xall=zeros(sg(2),iter);% solutions in all iterations          % 存放所有迭代的解：每列对应一次迭代
% 初始化解向量
X=zeros(sg(2),1); % initial solution                          % 初始解取全 0 向量（长度等于模型参数个数）
% 初始化残差与变化量记录数组
misfit=zeros(iter,1); % relative misfit                       % 归一化残差记录数组
%misfite=zeros(iter,1); % relative misfit                     % （保留原注释）另一种残差指标，未启用
Xdif=zeros(iter,1);                                           % 相邻两轮解相对变化量记录数组
obE=ob'*ob; % energy of data                                  % 观测数据（含约束右端项）的能量，用于归一化残差
% 下面计算初始残差与初始梯度
ex=ob-[G*X;D*X];% sg(1)*sg(2)                                 % 初始残差：把两块的残差按行堆叠 ex = ob - [G*X; D*X]
Xe=G'*ex(1:sg(1))+D'*ex(sg(1)+1:end);% sg(1)*sg(2)           % 初始梯度：分别取两块残差做 G'、D' 作用后相加
...
for i=1:iter                                                  % 逐次迭代，共 iter 次
    ...
    if isscaling==1                                           % 若开启了“解缩放”选项
        fac=ob'*syn./(syn'*syn);syn=syn*fac;X=X*fac; % scaled for rapid converging         % 缩放因子 fac=(ob'syn)/(syn'syn)，同步缩放以加速收敛
    end                                                       % 解缩放分支结束
    ex=ob-syn;                                                % 更新残差
    % 记录本轮的归一化残差
